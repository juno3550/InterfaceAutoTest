...
class Excel:
    ...
    def get_sheet(self, sheet_name):
        if sheet_name not in self.get_all_sheet():
            error("sheet名称【%s】不存在，切换sheet失败！" % sheet_name)
            return
    ...
    # 保存写入了测试结果的excel数据文件
    def save(self, file_save_name, timestamp):
        save_dir = os.path.join(TEST_REPORT_SAVE_PATH, get_chinese_date())
        if not os.path.exists(save_dir):
            os.mkdir(save_dir)
        excel_result_file_path = os.path.join(save_dir, file_save_name + "_" + timestamp + ".xlsx")
        self.wb.save(excel_result_file_path)
        return excel_result_file_path


if __name__ == "__main__":
    from util.global_var import *
    excel = Excel(EXCEL_FILE_PATH)
    excel.get_sheet("测试用例集")
    # print(excel.get_all_row_data())
    excel.write_row_data(["4", None, "失败"], None, True, "green")
    excel.save()